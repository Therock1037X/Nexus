...
export default function DoctorDashboard() {
  ...
  const [activeModal, setActiveModal] = useState(null); // 'request' | 'prescribe' | 'escalate' | 'admit'
  ...
  const openEscalateModal = (patient) => {
    setModalPatient(patient);
  ...
  const tabs = [
    ...
    { id: 'request', label: 'Request Resource', icon: Bed },
    { id: 'prescribe', label: 'Prescribe', icon: Pill },
    { id: 'escalate', label: 'Escalate', icon: Sparkles, badgeColor: 'bg-rose-50 text-rose-700' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            {currentUser?.specialty || 'General & Critical Care'} • Attending: <strong className="text-slate-800">{currentUser?.name || 'Dr. Ananya Sharma'}</strong>
          </p>
        </div>
      ...
      {/* Tab 1: Patients (My Patients vs All Patients) */}
      {activeTab === 'patients' && (
        <div className="space-y-6 animate-in fade-in duration-150">
          {/* Quick NLP Prompt Bar */}
          <NaturalLanguageRequestInput onParsed={handleNlpParsed} />

          {/* 4 Floating Availability Stat Cards */}
          ...
          {/* Patient List (My Patients vs All Patients) */}
          <PatientList
            patients={patients}
            currentDoctorId={currentUser?.id || 'doc-1'}
            currentDoctorName={currentUser?.name || 'Dr. Ananya Sharma'}
            ...
            <div className="space-y-4">
              <div className="clean-card p-5 space-y-3 bg-white">
                ...
                <ul className="text-xs text-slate-600 space-y-2.5 leading-relaxed font-medium">
                  <li className="flex items-start gap-2">
                    ...
                    <span><strong>Live Locking:</strong> Prevents two doctors from accidentally booking the same bed at the exact same moment.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5" />
                    <span><strong>Instant Sync:</strong> Once confirmed, the bed immediately updates across all staff screens in real time.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5" />
                    <span><strong>Emergency Override:</strong> Acute life-threatening cases can override standard bookings if necessary.</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="clean-card p-5 bg-emerald-50/50 border-emerald-200">
                <h4 className="text-xs font-bold text-emerald-900 flex items-center gap-1.5 mb-1.5 uppercase font-mono">
                  <Sparkles className="w-3.5 h-3.5 text-emerald-700" /> AI Suggestion
                </h4>
                <p className="text-xs text-slate-700 leading-relaxed font-medium">
                  Medical notes are analyzed by AI to recommend standard or critical urgency. You always retain final manual control.
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="clean-card p-5 space-y-3 bg-white">
                ...
                <div className="space-y-2 text-xs text-slate-600 font-medium">
                  <div className="p-3 rounded-xl bg-purple-50 border border-purple-200">
                    ...
                    <p className="text-[11px] text-purple-800 mt-0.5">Medicine quantity is reserved in pharmacy inventory.</p>
                  </div>
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                    <span className="font-bold text-slate-800">Step 2: Pharmacy Dispenses</span>
                    <p className="text-[11px] text-slate-600 mt-0.5">Pharmacist verifies batch and sends medicine to ward.</p>
                  </div>
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                    <span className="font-bold text-slate-800">Step 3: Nurse Administers</span>
                    <p className="text-[11px] text-slate-600 mt-0.5">Nurse administers dose at bedside and records vitals.</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6 animate-in fade-in duration-150">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 clean-card p-6 bg-white">
              ...
              <EscalateForm preselectedResource={modalPatient?.currentBedId ? { id: modalPatient.currentBedId } : null} />
            </div>
            ...
            <div className="space-y-4">
              <div className="clean-card p-5 space-y-3 bg-white">
                ...
                <ul className="text-xs text-slate-600 space-y-2.5 leading-relaxed font-medium">
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5" />
                    <span><strong>Clear Rules:</strong> When two requests compete for a bed, higher clinical urgency automatically receives it.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5" />
                    <span><strong>Staff Alert:</strong> The prior doctor is immediately notified to choose an alternate bed.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5" />
                    <span><strong>Action Logged:</strong> This action will be logged in the hospital activity history for safety and transparency.</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="bg-white max-w-xl w-full rounded-2xl p-6 border border-slate-200 shadow-xl relative max-h-[90vh] overflow-y-auto">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 mb-1 flex items-center gap-2">
                <Bed className="w-5 h-5 text-emerald-700" /> Allocate / Reserve Bed or Resource
              </h3>
              <p className="text-xs text-slate-500 mb-4 font-medium">
                Assign a bed, operating theatre, or equipment to a patient.
              </p>
              <RequestResourceForm
      ...
      {activeModal === 'prescribe' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white max-w-xl w-full rounded-2xl p-6 border border-slate-200 shadow-xl relative max-h-[90vh] overflow-y-auto">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 mb-1 flex items-center gap-2">
                <Pill className="w-5 h-5 text-purple-700" /> Prescribe Medication
              </h3>
              <p className="text-xs text-slate-500 mb-4 font-medium">
                Sends medication order directly to Central Pharmacy.
              </p>
              <PrescriptionForm
          ...
          <div className="bg-white max-w-xl w-full rounded-2xl p-6 border border-slate-200 shadow-xl relative max-h-[90vh] overflow-y-auto">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 mb-1 flex items-center gap-2 text-rose-700">
                <Sparkles className="w-5 h-5" /> Priority Override
              </h3>
              <p className="text-xs text-slate-500 mb-4 font-medium">
                Request priority override to reallocate a bed for an emergency patient.
              </p>
              <EscalateForm
                onSuccess={() => {
                  setTimeout(() => setActiveModal(null), 1000);
